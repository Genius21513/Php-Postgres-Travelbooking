...
<div>
            ...
            >
                <div class="p-8 bg-white border border-gray-100 shadow-md rounded-xl">
                    ...
                    <div class="items-center gap-3 my-2 md:flex">
                        ...
                        <div class="relative rounded-md" x-data="fromSelect();" x-init="init()">
                            ...
                            <input class="block py-3 pl-10 bg-gray-100 border-2 border-gray-300 rounded-md outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-md"
                                readonly id="from1" type="text" @click="isOpen = true" x-model="fromInput" placeholder="Leaving From" />
                            ...
                            <div class="absolute z-50 w-96" x-show="isOpen" x-trap="isOpen" style="display:none" @click.away="isOpen = false">
                                <div class="bg-white border rounded">
                                    <div class="flex p-4 text-gray-500">
                                        <input class="w-full px-4 text-gray-800 border-b-2 outline-none appearance-none"
                                        x-model="fromInput" @input.debounce.500ms.defer="search()" />
                                    </div>
                                    <div class="p-4 border-t-2 border-gray-100" x-data="{items : @entangle('airportcities')}">
                                        <template x-for="item in items">
                                            <div class="cursor-pointer group" @click="select(item.Code);">
                                                <div class="flex items-center pl-3 group-hover:bg-gray-300">
                                                    <template x-if="item.Type.toLowerCase() === 'airport'">
                                                    ...
                                                    <template x-if="item.Type.toLowerCase() === 'city'">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                                        <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                                                    </svg>                                                    
                                                    </template>
                                                    ...
                                                    <a class="block p-4 border-l-4 border-transparent ">
                                                        <p x-text="item.Code + ' - ' + item.Name"></p>
                                                        <p x-text="item.City + ', ' + item.Country" class="text-sm text-gray-600"></p>
                                                    </a>
                                                </div>
                                            </div>
                                        </template>
                        ...
                        <div class="relative rounded-md" x-data="toSelect()" x-init="init()">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                    <path fill-rule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clip-rule="evenodd" />
                                </svg>
                            ...
                            <input class="block py-3 pl-10 bg-gray-100 border-2 border-gray-300 rounded-md outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-md"
                            placeholder="Going To" readonly id="to1" type="text" @click="isOpen = true" x-model="toInput" />
                ...
                <div class="p-8 mt-2 bg-white border border-gray-100 shadow-md rounded-xl">
    ...
    <script>
        ...
        //Trip Select
        function tripSelect() {
            return { isOpen : false, trips : ["One way", "Round trip", "Multi-city"], tripInput :  "One way",
                init() { trip = this.tripInput; },
                change(t) { this.tripInput = t; trip = t; },
            }
        }

        //Plan Select
        function planSelect() {
            return { isOpen : false, plans : ["Economy", "Premium economy", "Business", "First"], planInput :  "Economy",
                init() { plan = this.planInput; },
                change(p) { this.planInput = p; plan = p; },
            }
        }
        ...
        function memberSelect() {
            return { isOpen : false,
                ...
                init() { membs = JSON.parse(JSON.stringify(this.members)); },
                getNumbers(short) {
                    switch (short) {
                        case 'adu': return this.members.adu; break;
                        case 'chi': return this.members.chi; break;
                        case 'ifs': return this.members.ifs; break;
                        case 'ifl': return this.members.ifl; break;
                    }
                },
        ...
        // Leaving From Select
        function fromSelect() {
            return { isOpen: false, fromInput: '',
                init() { },
                search() {
                    Livewire.emit('searchAirportCity', this.fromInput);
                    // Livewire.on('searchAirportCity', name => {
                    // })
                },
                select(t) { this.fromInput = t; this.isOpen = false; from1 = t; }
            }
        }
